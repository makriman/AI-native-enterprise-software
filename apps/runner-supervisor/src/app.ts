...
export async function createApp(options: RunnerSupervisorOptions = {}): Promise<FastifyInstance> {
  const token =
    options.token !== undefined ? optionalSecret(options.token) : optionalSecret(process.env.RUNNER_SUPERVISOR_TOKEN);
  ...
  app.addHook("onRequest", async (request, reply) => {
    ...
    if (!decision.ok) {
      if (decision.status === 503) {
        request.log.error({ path: pathName }, "refusing request because RUNNER_SUPERVISOR_TOKEN is not configured");
      }
